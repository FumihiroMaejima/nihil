...
export default class AuthApp {
  ...
  public async login(email: string, password: string): Promise<boolean> {
    const response = await this.authentication.loginRequest({
      email: email,
      password: password,
    })
  ...
  public async logout(): Promise<boolean> {
    const response = await this.authentication.logoutRequest(
      this.getHeaderOptions().headers
    )
  ...
  public getHeaderOptions(): AuthAppHeaderOptions {
    ...
    return {
      headers: this.addHeaders({
        id: this.getAuthId(),
        authority: String(this.getAuthAuthority()),
        token: token,
      }),
   ...
   * @return {void}
   */
  protected setCookie(key: string, value: string, minutes = 10) {
   ...
   * @return {void}
   */
  protected restoreToken(token = '', isRestore = false) {
    isRestore
      ? this.setCookie(this.appKey, token)
      : this.removeCookie(this.appKey)
  }
}
